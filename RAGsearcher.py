...
def query_related_titles(question,embedded_path,count=5):
    ...
    similarities = similarities * (np.array([penalize(len(text)) for text in embedded_texts]))  # Penalize longer texts
...
def text_to_feed(doc_embedded_path,ref_embedded_path,pure_doc,pure_ref,question,countDocu=5,countRef=3):
    related_texts_doc_idx = query_related_titles(question,doc_embedded_path,countDocu)
    related_texts_ref_idx = query_related_titles(question,ref_embedded_path,countRef)
...
if __name__ == "__main__":
    ...
    with open("embeddings\\doc_text.json", "r") as file:
        pure_doc = json.load(file)
    with open("embeddings\\ref_text.json", "r") as file:
        pure_ref = json.load(file)
    toFeed=text_to_feed(doc_embedded_path,ref_embedded_path,pure_doc,pure_ref,question)
