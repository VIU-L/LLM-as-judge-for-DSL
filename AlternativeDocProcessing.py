...
def parse_md_file(file_path):
    ...
    for subtitle_section in subtitles:
        ...
        subsubtitles = re.findall(r"(### .+?)(?=\n### |\Z)", subtitle_section, re.DOTALL)
        
        if subsubtitles:
            # Extract content before the first subsubtitle (if any)
            pre_subsubtitle_content = subtitle_section.split("###")[0].split("\n", 1)
            if len(pre_subsubtitle_content) > 1 and pre_subsubtitle_content[1].strip():
                results.append(f"{subtitle_title} &Content: {pre_subsubtitle_content[1].strip()}")
            ...
            for subsubtitle_section in subsubtitles:
                ...
                subsubtitle_content = " ".join(subsubtitle_lines[1:]).strip() if len(subsubtitle_lines) > 1 else "No content"
                results.append(f"{subtitle_title} {subsubtitle_title} &Content: {subsubtitle_content}")
        else:
            # If no subsubtitles, include all content in the subtitle
            subtitle_content = " ".join(subtitle_lines[1:]).strip() if len(subtitle_lines) > 1 else "No content"
    ...
    for file_name in os.listdir(folder_path):
        if file_name.endswith(".md"):
            ...
            for result in results:
                texts.append("&Title: " +file_name[:-3]+" &Subtitle: " + result)
    return texts
...
if __name__ == "__main__":
    ...
    save_embedding(doc_texts,"embeddings\\doc_embedded.pt","embeddings\\doc_text.json")
    
    ref_texts=parse_folder('reference\\reference')
    save_embedding(ref_texts,"embeddings\\ref_embedded.pt","embeddings\\ref_text.json")
